      ...
      <div className="rounded-2xl border border-border bg-card p-8">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <Link to="/companies/$slug" params={{ slug: job.companies?.slug ?? "" }} className="mt-1 inline-block text-muted-foreground hover:text-primary">
              {job.companies?.name}
            </Link>
          </div>
        ...
        <div className="mt-6 flex flex-wrap gap-3 text-sm text-muted-foreground">
          ...
          <span className="flex items-center gap-1.5 capitalize"><Briefcase className="h-4 w-4" />{job.job_type.replace("_", " ")}</span>
          {(job.salary_min || job.salary_max) && (
            <span className="flex items-center gap-1.5"><DollarSign className="h-4 w-4" />{job.salary_min}–{job.salary_max} {job.salary_currency}</span>
          )}
        ...
        <div className="mt-8 flex gap-3 border-t border-border pt-6">
          <Button size="lg" onClick={apply}>Apply now</Button>
        </div>
      </div>
    </div>
  );
