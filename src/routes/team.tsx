          ...
          {data?.map((m) => (
            <div key={m.id} className="bg-background p-6">
              ...
              <div className="mt-3 flex gap-3">
                {m.email && <a href={`mailto:${m.email}`} className="text-muted-foreground hover:text-foreground"><Mail className="h-4 w-4" /></a>}
                {m.linkedin && <a href={m.linkedin} className="text-muted-foreground hover:text-foreground"><Linkedin className="h-4 w-4" /></a>}
              </div>
            </div>
          ))}
        </div>
